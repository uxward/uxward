...
- [x] **~~Five mobile breakpoints.~~ Consolidated 2026-08-29.** 600 and 640 were one-file inventions; 880 sat 20px off the real tablet switch. Folded into **720** (mobile) and **900** (tablet) — 19 media queries, two breakpoints. Verified the 1 → 2 → 3 column progression lands exactly on those two widths.
- [x] **~~Archive stuck at three columns from 721px down.~~** Found while verifying the above: `.arch-grid` was declared *after* the 900px block, so the base rule won on source order and the collapse to two columns never fired. Pre-existing. All media queries in that file now sit at the end, and DESIGN.md carries the rule.
